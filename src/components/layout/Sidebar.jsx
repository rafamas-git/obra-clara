...
const ICONS = {
  ...
  switch:   <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>,
}

export default function Sidebar() {
  const { profile, rolEnObra, misObras, obraActual, signOut } = useAuth()
    ...
    <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-100 h-screen sticky top-0 p-4">
      ...
      <div className="flex items-center gap-3 px-3 mb-6">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-1 leading-none truncate">{seccionActiva}</p>
        </div>
      </div>

      {/* Selector de obra (solo si tiene más de una) */}
      {misObras.length > 1 && obraActual && (
        <button
          onClick={() => navigate('/seleccionar-obra')}
          className="flex items-center gap-2 mx-3 mb-4 px-3 py-2 rounded-xl bg-gray-50 hover:bg-primary-50 text-left transition-colors group"
        >
          <span className="w-4 h-4 text-gray-400 group-hover:text-primary-600 flex-shrink-0">
            {ICONS.switch}
          </span>
